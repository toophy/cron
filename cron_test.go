...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("1", "* * * * * *", func() { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("1", "* * * * * ?", func() { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("1", "@every 2s", func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("1", "0 0 0 1 1 ?", func() {})
	cron.AddFunc("2", "* * * * * ?", func() { wg.Done() })
	id1, _ := cron.AddFunc("3", "* * * * * ?", func() { t.Fatal() })
	id2, _ := cron.AddFunc("4", "* * * * * ?", func() { t.Fatal() })
	cron.AddFunc("5", "0 0 0 31 12 ?", func() {})
	cron.AddFunc("6", "* * * * * ?", func() { wg.Done() })
...
func TestRunningJobTwice(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("1", "0 0 0 1 1 ?", func() {})
	cron.AddFunc("2", "0 0 0 31 12 ?", func() {})
	cron.AddFunc("3", "* * * * * ?", func() { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("2", "0 0 0 31 12 ?", func() {})
	cron.AddFunc("3", "* * * * * ?", func() { wg.Done() })
	cron.Schedule("4", Every(time.Minute), FuncJob(func() {}))
	cron.Schedule("5", Every(time.Second), FuncJob(func() { wg.Done() }))
	cron.Schedule("6", Every(time.Hour), FuncJob(func() {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("1", spec, func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestInvalidJobSpec(t *testing.T) {
	cron := New()
	_, err := cron.AddJob("1", "this will not parse", nil)
	if err == nil {
...
func TestJob(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddJob("1", "0 0 0 30 Feb ?", testJob{wg, "job0"})
	cron.AddJob("2", "0 0 0 1 1 ?", testJob{wg, "job1"})
	job2, _ := cron.AddJob("3", "* * * * * ?", testJob{wg, "job2"})
	cron.AddJob("4", "1 0 0 1 1 ?", testJob{wg, "job3"})
	cron.Schedule("5", Every(5*time.Second+5*time.Nanosecond), testJob{wg, "job4"})
	job5 := cron.Schedule("6", Every(5*time.Minute), testJob{wg, "job5"})
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule("1", Every(time.Hour), FuncJob(func() {}))
	cron.Schedule("2", Every(time.Second), FuncJob(func() {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("1", "* * * * * *", func() { atomic.AddInt64(&calls, 1) })
	cron.Schedule("2", new(ZeroSchedule), FuncJob(func() { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("1", "* * * * * *", func() {})
		cron.Start()
		cron.AddFunc("2", "* * * * * *", func() { time.Sleep(2 * time.Second) })
		cron.AddFunc("3", "* * * * * *", func() {})
		time.Sleep(time.Second)
